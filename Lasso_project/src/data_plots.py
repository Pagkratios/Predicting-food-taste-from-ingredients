...
# Old (legacy) outputs preserved for comparison
OLD_PLOTS_DIR = os.path.join(PROJECT_ROOT, "results", "old_plots", "plots_data")
OLD_TSNE_DIR  = os.path.join(PROJECT_ROOT, "results", "old_plots", "t-sne")
OLD_PCA_DIR   = os.path.join(PROJECT_ROOT, "results", "old_plots", "pca")

for _d in (PLOTS_DIR, TSNE_DIR, PCA_DIR, OLD_PLOTS_DIR, OLD_TSNE_DIR, OLD_PCA_DIR):
...
def plot_ingredient_usage_pie_v2(raw_recipes, outdir, threshold=3):
    """Pie chart of ingredient usage with unified styling."""
    from plot_config import setup_figure, save_figure, FONT_SIZE_TITLE, FONT_SIZE_LEGEND
    ...
    wedges, _ = ax.pie(
        sizes, startangle=90, counterclock=False, colors=colors,
        wedgeprops={"edgecolor": "white", "linewidth": 0.6, "antialiased": True},
    )
    ax.axis("equal")

    legend_handles = [
        mpatches.Patch(color=colors[i], label=f"{label}: {size:.1f}% ({count})")
        for i, (label, size, count) in enumerate(zip(labels, sizes, counts_only))
    ]
    ax.legend(
        handles=legend_handles, loc="center left", bbox_to_anchor=(1, 0.5),
        title="Ingredients", frameon=True, fancybox=True, fontsize=FONT_SIZE_LEGEND,
    )
    ax.set_title(
        f"Ingredient Usage Across {total_recipes} Recipes (\u2265{threshold} occurrences)",
...
def plot_gaussian_per_key_v2(raw_recipes, outdir, top_k=3):
    ...
    from plot_config import (
        setup_figure, save_figure, style_axes, get_sensory_color,
        SENSORY_ORDER as SO, FONT_SIZE_ANNOTATION, FONT_SIZE_TITLE, FONT_SIZE_LABEL,
    )

    x_grid = np.linspace(0, 100, 1000)
    text_pe = [pe.withStroke(linewidth=1.8, foreground="white")]
    ...
    for key in SO:
        ...
        if vals.size and sd > 0:
            ...
            for rank, (i, xoff) in enumerate(zip(order, x_jitter)):
                x0 = float(vals[i])
                y0 = float(kde([x0])[0]) if kde is not None else 0.0
                y_label = y0 + y_curve_max * 0.22
                y_label = min(y_label, y_curve_max * (1.0 + 0.35) * 0.98)

                ax.annotate(
                    names[i], xy=(x0, y0), xytext=(x0 + xoff, y_label),
                    textcoords='data', ha='center', va='bottom',
                    fontsize=FONT_SIZE_ANNOTATION, color='black', fontweight='bold',
                    path_effects=text_pe,
                    arrowprops=dict(arrowstyle='-', color='black', lw=0.8, alpha=0.9),
                    zorder=4, annotation_clip=False,
                )
                ax.axvline(x0, color='black', linestyle=':', alpha=0.7, linewidth=0.8, zorder=2.5)

        ax.set_xlim(0, 100)
        y_top = max(y_curve_max * 1.35, ax.get_ylim()[1])
        ax.set_ylim(0, y_top)

        ax.set_title(f"{key.capitalize()} Distribution + Gaussian Fit",
...
def plot_combined_kde_v2(raw_recipes, outdir):
    """Combined KDE of all sensory keys (excl. sour) + pooled Gaussian."""
    from plot_config import (
        setup_figure, save_figure, style_axes, get_sensory_color,
    ...
    ax.set_title('Sensory Distribution Across Recipes', fontsize=FONT_SIZE_TITLE, weight='bold')
    ax.set_xlabel('Score (0\u2013100)', fontsize=FONT_SIZE_LABEL)
    ax.set_ylabel('Density', fontsize=FONT_SIZE_LABEL)
...
def _pick_k_silhouette_v2(X, k_min=2, k_max=12, plot_path=None, seed=42):
    ...
    from plot_config import setup_figure, save_figure, style_axes, FONT_SIZE_TITLE, FONT_SIZE_LABEL
    ...
    if plot_path:
        ...
        ax.set_title("Silhouette (KMeans)", fontsize=FONT_SIZE_TITLE)
...
def _render_tsne_v2(Z, labels, recipe_names, ingredient_names, n_rec, out_path):
    ...
    from plot_config import setup_figure, save_figure, style_axes
...
def pca_clusters_v2(raw_recipes, n_components=2, standardize=False, outdir=None, k_min=2, k_max=12, seed=42):
    ...
    from plot_config import (
        setup_figure, save_figure, style_axes, FONT_SIZE_TITLE, FONT_SIZE_LABEL,
    )
    ...
    ax.set_title("PC1 Loadings by Feature", fontsize=FONT_SIZE_TITLE)
    save_figure(fig, os.path.join(outdir, "pc1_loadings.png"))
    ...
    if n_samples < 3:
        ...
        ax.set_ylabel("Silhouette score", fontsize=FONT_SIZE_LABEL)
        ax.set_title("Silhouette Scores (PCA space)", fontsize=FONT_SIZE_TITLE)
        if len(K_range) > 0:
            ax.set_xticks(K_range)
        save_figure(fig, os.path.join(outdir, "pca_silhouette.png"))
    ...
    ax.set_title(f"PCA (k={best_k})", fontsize=FONT_SIZE_TITLE)
    ...
    save_figure(fig, os.path.join(outdir, "pca_bestk_scatter.png"))
...
def main():
    ...
    # ── Legacy plots (preserved in old_plots/ for comparison) ──
    plot_ingredient_usage_pie(raw_recipes, OLD_PLOTS_DIR, threshold=3)
    plot_gaussian_per_key(raw_recipes, OLD_PLOTS_DIR, top_k=3)
    plot_combined_kde_with_gaussian(raw_recipes, OLD_PLOTS_DIR)
    pca_clusters_minimal(raw_recipes, n_components=2, standardize=False, outdir=OLD_PCA_DIR)
    run_tsne_bestk_and_stats(raw_recipes, outdir=OLD_TSNE_DIR)
    print("[+] Legacy plots saved to results/old_plots/")
    ...
    plot_combined_kde_v2(raw_recipes, PLOTS_DIR)
